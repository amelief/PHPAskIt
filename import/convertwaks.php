...
####### IMPORT QUESTIONS FROM Wak's Ask&Answer (formerly from luved.org) TO Askably 3.1 #######
...
3. PLACE THIS FILE IN THE import DIRECTORY OF YOUR Askably INSTALLATION (_NOT_ YOUR Wak's Ask&Answer DIRECTORY).
...
foreach (getTotalAnswered() as $waksquestions) {
	...
	$pai_db->query('INSERT INTO `' . $pai_db->getTable() . "` VALUES ('', '" . $question . "', '" . $answer . "', 1, '" . $dateasked . "', '" . $ip . "');") or exit('Couldn\'t add question #' . $id . 'to the database... Stopping import process.');
